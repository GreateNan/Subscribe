...
App({
  ...
  //添加一个新的地址
  addAddress: function (setName, infoObject, callback) {
    const db = wx.cloud.database()
    db.collection(setName).add({
      data: infoObject,
      success: callback,
  ...
  //修改地址信息
  updataAddress: function (setName, tagretInfo, infoObject, callback) {
    const db = wx.cloud.database()
    db.collection(setName).doc(tagretInfo).update({
      data: infoObject,
      success: callback,
      fail: console.error
    })
  ...
  // 从地址表里获取用户的地址信息
  getAddressInfoWhere: function (setName, ruleObj, callback) {
    const db = wx.cloud.database()
  ...
  // 从集合中筛选数据
  getInfoWhere2: function (setName, ruleObj,ruleObjb, callback) {
    const db = wx.cloud.database()
    const _ = db.command
    console.log(ruleObj)
    console.log(ruleObjb)
...
// 排序后取出数据
getInfoByOrder2: function (setName, ruleItem, orderFuc,status, callback) {
