    ...
    wx.cloud.callFunction({
      ...
      complete: res => {
        if (res.result.openId) {
          ...
          app.getAddressInfoWhere('address', {
              '_openid': openid
            },
            e => {
              if (e.data.length > 0) {
...
toPay(){
  ...
  if (that.data.hasAddress){
    ...
    var tmp={};
    tmp['xxAddress'] = that.data.address['xxAddress']
    tmp['name'] = that.data.address['name']
    tmp['phone'] = that.data.address['phone']
    tmp['address'] = that.data.address['address']
  ...
  toPay1() {
    ...
    if (that.data.hasAddress) {
      ...
      var p = new Promise((resolve, reject) => {
        ...
        tmp['xxAddress'] = that.data.address['xxAddress']
        tmp['name'] = that.data.address['name']
        tep['phone'] = that.data.address['phone']
        tep['address'] = that.data.address['address']
        tmp['orderTime'] = app.CurrentTime_show()
        tmp['orderSuccess'] = true
        tmp['payTime'] = ''
        tmp['paySuccess'] = false
